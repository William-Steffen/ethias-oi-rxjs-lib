...
public class FilterValidator implements ConstraintValidator<ValidFilter, String> {
     ...
     */
    private boolean parseNode(final Node node) {
        final List<Field> fields = parseClass(clazz);
        if (node instanceof ComparisonNode) {
            final String[] selectors = ((ComparisonNode) node).getSelector().split("\\.");
            return compareFields(fields, selectors);

        } else if (node instanceof LogicalNode) {
            return ((LogicalNode) node).getChildren()
                    .stream()
     ...
     */
    private boolean compareFields(final List<Field> fields, final String[] selectors) {
        final Optional<Field> optional = fields
                .stream()
     ...
     */
    private boolean compareNestedObjects(final Field field, final String[] selectors) {
        if (selectors.length < 2) {
            sendInvalidNestedObjectMessage(selectors[0], field.getDeclaringClass().getSimpleName());
            return false;
        }
        final List<Field> nestedFields = parseClass(field.getType());
     ...
     */
    private List<Field> parseClass(final Class<?> clazz) {
        final List<Field> fields = new ArrayList<>(List.of(clazz.getDeclaredFields()));
     ...
     */
    private void sendInvalidNestedObjectMessage(final String selector, final String className) {
     ...
     * @param selector The selector mentioned in the field
     */
    private void sendInvalidFieldMessage(final String selector) {
